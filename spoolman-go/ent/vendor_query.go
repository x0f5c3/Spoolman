...
func (vq *VendorQuery) QueryFilaments() *FilamentQuery {
	query := (&FilamentClient{config: vq.config}).Query()
...
func (vq *VendorQuery) QueryExtra() *VendorFieldQuery {
	query := (&VendorFieldClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VendorQuery) Clone() *VendorQuery {
	...
	return &VendorQuery{
		...
		withFilaments: vq.withFilaments.Clone(),
		withExtra:     vq.withExtra.Clone(),
...
func (vq *VendorQuery) WithFilaments(opts ...func(*FilamentQuery)) *VendorQuery {
	query := (&FilamentClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withFilaments = query
	return vq
}
...
func (vq *VendorQuery) WithExtra(opts ...func(*VendorFieldQuery)) *VendorQuery {
	query := (&VendorFieldClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//
//	client.Vendor.Query().
//		GroupBy(vendor.FieldRegistered).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (vq *VendorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Vendor, error) {
	...
	if query := vq.withFilaments; query != nil {
		if err := vq.loadFilaments(ctx, query, nodes,
			func(n *Vendor) { n.Edges.Filaments = []*Filament{} },
			func(n *Vendor, e *Filament) { n.Edges.Filaments = append(n.Edges.Filaments, e) }); err != nil {
			return nil, err
		}
	}
	if query := vq.withExtra; query != nil {
		if err := vq.loadExtra(ctx, query, nodes,
			func(n *Vendor) { n.Edges.Extra = []*VendorField{} },
			func(n *Vendor, e *VendorField) { n.Edges.Extra = append(n.Edges.Extra, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VendorQuery) loadFilaments(ctx context.Context, query *FilamentQuery, nodes []*Vendor, init func(*Vendor), assign func(*Vendor, *Filament)) error {
	...
	query.Where(predicate.Filament(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(vendor.FilamentsColumn), fks...))
	}))
...
func (vq *VendorQuery) loadExtra(ctx context.Context, query *VendorFieldQuery, nodes []*Vendor, init func(*Vendor), assign func(*Vendor, *VendorField)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Vendor)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.VendorField(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(vendor.ExtraColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
